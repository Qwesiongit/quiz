...
function logoff(req,res,next){
    ...
    function register(req,res){
     ...
     let {fullname,email,password,level}=req.body;
       ...
       switch (type) {
           case "general":
              General.find().then(g_courses=>{
                if(!(g_courses==null)){
                    //console.log(g_courses);
                    g_courses.forEach(element => {
                        if(element.level.includes(level)){
                        this_courses.push(element);
                    }});
                    return res.send({success:true,courses:this_courses});
                   }else{
               ...
               case "past_question":
                ...
                then(p_courses=>{
                    if(!(p_courses==null)){
                        if(level==="shs"){
                            p_courses.forEach(element => {
                                if(element.level.includes("wassce")){
                                this_courses.push(element);
                            }});
                        }
                        if(level==="jhs"){
                            p_courses.forEach(element => {
                                if(element.level.includes("bece")){
                                this_courses.push(element);
                            }});
                        }

                        if(level==="teacher promotion"){
                            p_courses.forEach(element => {
                                if(element.level.includes("teacher promotion")){
                                this_courses.push(element);
                            }});
                        }
                        return res.send({success:true,courses:this_courses});
...
const getpay_records= (req,res)=>{
   ...
   app.post('/api/user/getcourses',isloggedin,showCourses);

   app.post('/api/user/getcourses_',showCourses_);
