...
module.exports = function(app) {
    ...
    const cataddValidation=[
        ...
        check('hr').not().isEmpty().withMessage("hour is required"),
        check('min').not().isEmpty().withMessage("minutes is required"),
        check('sec').not().isEmpty().withMessage("secons is required")
    ];
  ...
  function islogged(req,res){
       ...
       function addCategory(req,res){
        ...
        if(req.session.user && req.session.user.user_type==="admin"){
            switch (req.body.type) {
                case "past_question":
                    let {year,level,exam_type,course,short_name,min,sec,hr}=req.body;
                    let _d = {
                        'hr':hr,
                        'min':min,
                        'sec':sec
    ...
    function register(req,res){
     ...
     admin_type==="super"?admin.active="yes":admin.active="no";
